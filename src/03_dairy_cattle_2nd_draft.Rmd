...
$$\text{Table 3.5.1: Pairwise comparison tests}$$
...
$$\text{Table 4.1: Spearman's rank correlation test result on 5 different comparisons using VAH algorithm.}$$
...
```{r elo-score-vah-algo, message=FALSE, echo=FALSE, warning=FALSE}
...
sub_intersection <- Reduce(intersect, list(water_high_cows, water_low_cows, 
                                           feed_high_cows, feed_low_cows))
...
```{r Results-correlation 1, echo=FALSE, message=FALSE, results='hide'}
 # ----------- Test 1 water_feed ------------------------------------
  # feed bins overall
...
```
```{r Results-correlation plots 1, echo=FALSE, message=FALSE, fig.align='center', out.width="80%"}
  #plot the water_feed graph
  df_f_w <- data.frame(x = rank(unname(sorted_last_elo_feed), ties.method = 'average'), y = rank(unname(sorted_last_elo_water), ties.method = 'average'))
  sp_df_f_w <- ggscatter(df_f_w, x = 'x', y = 'y',
   add = "reg.line",  # Add regression line
   add.params = list(color = "blue", fill = "lightgray"),
   conf.int = TRUE, # Add confidence interval
   cor.coeff.args = list(method = "spearman", label.sep = "\n"),
   xlab = "Dominance ranks of cows in Feed Bin group",
   ylab = "Dominance ranks of cows in Water Bin group"
   ) + labs(caption = "Figure 4.1: Correlation between the dominance ranks at Feed Bin and Water Bin (Overall)")
sp_df_f_w
```
...
```{r Results-correlation 2.1, echo=FALSE, message=FALSE, results='hide'}
# ----------- Test 2 water_feed_high_cor ------------------------------------
  #feed bins high THI 
  ...
  df_fh_wh <- data.frame(x = rank(unname(sorted_last_elo_feed_high_THI), 
                                  ties.method = 'average'), 
                         y = rank(unname(sorted_last_elo_water_high_THI), 
                                  ties.method = 'average'))
  sp_df_fh_wh <- ggscatter(df_fh_wh, x = 'x', y = 'y',
   add = "reg.line",  # Add regression line
   add.params = list(color = "blue", fill = "lightgray"), 
   conf.int = TRUE, # Add confidence interval
   cor.coeff.args = list(method = "spearman"),
   xlab = "Dominance ranks of cows in Feed Bin High THI group",
   ylab = "Dominance ranks of cows in Water Bin High THI group"
   ) + 
    labs(caption = "Figure 4.2: Correlation between the dominance ranks at Feed Bin and Water Bin under High THI")
  
  
   # ----------- Test 3 water_feed_low_cor ------------------------------------
  
  #feed bins low THI
  sub_repl <- feed_low[sample(1:nrow(water_low)),]
  sub_repl$index <- seq(1, nrow(sub_repl))
  ## Order replacements
  elo_repl_list <- sub_repl[order(sub_repl$hour),c("Actor_cow", 
                                                   "Reactor_cow",
                                                   "hour", "Bin", "date")]
  colnames(elo_repl_list)=c("winner","loser","time","bin", "date")
  elo_repl_list$index <- seq(1, nrow(elo_repl_list))
  ...
  df_fl_wl <- data.frame(x = rank(unname(sorted_last_elo_feed_low_THI), 
                                  ties.method = 'average'), 
                         y = rank(unname(sorted_last_elo_water_low_THI), 
                                  ties.method = 'average'))
  
  sp_df_fl_wl <- ggscatter(df_fl_wl, x = 'x', y = 'y',
   add = "reg.line",  # Add regression line
   add.params = list(color = "blue", fill = "lightgray"),
   conf.int = TRUE, # Add confidence interval
   cor.coeff.args = list(method = "spearman", label.sep = "\n"),
   xlab = "Dominance ranks of cows in Feed Bin Low THI group",
   ylab = "Dominance ranks of cows in Water Bin Low THI group"
   ) + 
    labs(caption = "Figure 4.3: Correlation between the dominance ranks at Feed Bin and Water Bin under Low THI")
```
  

```{r Results-correlation plots 2, echo=FALSE, fig.show='hold', out.width="50%"}
sp_df_fh_wh  
sp_df_fl_wl
```
...
```{r Results-correlation 3, echo=FALSE, message=FALSE, results='hide'}
# ----------- Test 4 water_high_low_cor ------------------------------------
  #water bins high THI
  sub_repl <- water_high
  sub_repl$index <- seq(1, nrow(sub_repl))
  ## Order replacements
 ...
 df_wh_wl <- data.frame(x = rank(unname(last_elo_water_high_THI[sort(water_cows)]), ties.method = 'average'), y = rank(unname(last_elo_water_low_THI[sort(water_cows)]), ties.method = 'average'))
  
  sp_df_wh_wl <- ggscatter(df_wh_wl, x = 'x', y = 'y',
     add = "reg.line",  # Add regression line
     add.params = list(color = "blue", fill = "lightgray"),
     conf.int = TRUE, # Add confidence interval
     cor.coeff.args = list(method = "spearman", label.sep = "\n"),
     xlab = "Dominance ranks of cows in Water Bin High THI group",
     ylab = "Dominance ranks of cows in Water Bin Low THI group"
     ) + labs(caption = "Figure 4.4: Correlation between the dominance ranks at Water Bin under High THI vs under Low THI")

  
  # ----------- Test 5 feed_high_low_cor ------------------------------------
  #feed bins low THI
  ...
  df_fh_fl <- data.frame(x = rank(unname(last_elo_feed_high_THI[sort(feed_cows)]), ties.method = 'average'), y = rank(unname(last_elo_feed_low_THI[sort(feed_cows)]), ties.method = 'average'))
  sp_df_fh_fl <- ggscatter(df_fh_fl, x = 'x', y = 'y',
     add = "reg.line",  # Add regression line
     add.params = list(color = "blue", fill = "lightgray"),
     conf.int = TRUE, # Add confidence interval
     cor.coeff.args = list(method = "spearman", label.sep = "\n"),
     xlab = "Dominance ranks of cows in Feed Bin High THI group",
     ylab = "Dominance ranks of cows in Feed Bin Low THI group"
     ) + labs(caption = "Figure 4.5: Correlation between the dominance ranks at Feed Bin under High THI vs under Low THI")

```

```{r Results-correlation plots 3, echo=FALSE, fig.show='hold', out.width="50%"}
sp_df_wh_wl  
sp_df_fh_fl
```
...
sig_water_feed_cor = 0;
sig_water_feed_high_cor = 0;
sig_water_feed_low_cor = 0;
sig_water_high_low_cor = 0;
sig_feed_high_low_cor = 0;
...
for (i in 1:num) {
  ...
  sorted_last_elo_feed <- last_elo_feed[sort(cows)]
  sorted_last_elo_water <- last_elo_water[sort(cows)]
  water_feed_test <- cor.test(x=unname(sorted_last_elo_water), 
                              y=unname(sorted_last_elo_feed), 
                              method = 'spearman')
  water_feed_cor <- water_feed_cor + water_feed_test$estimate
  sig_water_feed_cor <- sig_water_feed_cor + (water_feed_test$p.value < 0.05)
  ...
  sub_repl$index <- seq(1, nrow(sub_repl))
  ## Order replacements
  elo_repl_list <- sub_repl[order(sub_repl$hour),
                            c("Actor_cow", "Reactor_cow",
                                                   "hour", "Bin", "date")]
  colnames(elo_repl_list)=c("winner","loser","time","bin", "date")
  elo_repl_list$index <- seq(1, nrow(elo_repl_list))
  #Calculate the ELO scores 
  ...
  last_elo_feed_high_THI <- extract_elo(elo_res_feed_high_THI)
  high_THI_cows <- 
    Reduce(intersect, list(names(last_elo_water_high_THI),
  ...
  water_feed_high_test <- cor.test(x=unname(sorted_last_elo_feed_high_THI), 
                                   y=unname(sorted_last_elo_water_high_THI), 
                                   method = 'spearman')
  water_feed_high_cor <- water_feed_high_cor + water_feed_high_test$estimate
  sig_water_feed_high_cor <- sig_water_feed_high_cor + 
    (water_feed_high_test$p.value < 0.05)
  ...
  sorted_last_elo_feed_low_THI <- last_elo_feed_low_THI[sort(low_THI_cows)]
  sorted_last_elo_water_low_THI <- last_elo_water_low_THI[sort(low_THI_cows)]
  water_feed_low_test <- cor.test(x=unname(sorted_last_elo_feed_low_THI), 
                                  y=unname(sorted_last_elo_water_low_THI),
                                  method = 'spearman')
  water_feed_low_cor <- water_feed_low_cor + water_feed_low_test$estimate
  sig_water_feed_low_cor <- sig_water_feed_low_cor + 
    (water_feed_low_test$p.value < 0.05)
  ...
  last_elo_water_low_THI <- extract_elo(elo_res_water_low_THI)
  last_elo_water_high_THI <- extract_elo(elo_res_water_high_THI)
  water_cows <- Reduce(intersect, 
                       list(names(last_elo_water_low_THI),
  ...
  water_high_low_test <- 
    cor.test(x=unname(last_elo_water_low_THI[sort(water_cows)]), 
             ...
             method = 'spearman')
  water_high_low_cor <- water_high_low_cor + water_high_low_test$estimate
  sig_water_high_low_cor <- sig_water_high_low_cor + 
    (water_high_low_test$p.value < 0.05)
  ...
  last_elo_feed_low_THI <- extract_elo(elo_res_feed_low_THI)
  last_elo_feed_high_THI <- extract_elo(elo_res_feed_high_THI)
  feed_cows <- Reduce(intersect, 
                      list(names(last_elo_feed_low_THI),
                           names(last_elo_feed_high_THI)))
  feed_high_low_test <- 
    cor.test(x=unname(last_elo_feed_low_THI[sort(feed_cows)]), 
                                 y=unname(
                                   last_elo_feed_high_THI[sort(feed_cows)]),
             method = 'spearman')
  feed_high_low_cor <- feed_high_low_cor + feed_high_low_test$estimate
  sig_feed_high_low_cor <- sig_feed_high_low_cor 
  + (feed_high_low_test$p.value < 0.05)
}
...
sig_water_feed_cor
sig_water_feed_high_cor
sig_water_feed_low_cor
sig_water_high_low_cor
sig_feed_high_low_cor
```

### Calculate fisher's z transformation score
```{r fisher-z-transformation, message=TRUE, eval=FALSE}
# -----Test 2 final_water_feed_high_cor & 3 final_water_feed_low_cor ------
library(DescTools)
# ------ COMPARING BETWEEN TEST 2 RHO VS. TEST 3 RHO -----------
n1 <- length(sorted_last_elo_feed_high_THI) + 
  length(sorted_last_elo_water_high_THI)
n2 <- length(sorted_last_elo_feed_low_THI) + 
  length(sorted_last_elo_water_low_THI)
z_final_water_feed_high_cor <- FisherZ(final_water_feed_high_cor)
z_final_water_feed_low_cor <- FisherZ(final_water_feed_low_cor)
zobs_test23 <-
  (z_final_water_feed_high_cor-z_final_water_feed_low_cor)/
  sqrt(1/(n1-3)+1/(n2-3))
...
n1<- length(last_elo_water_low_THI[sort(water_cows)]) + 
  length(last_elo_water_high_THI[sort(water_cows)])
n2 <- length(last_elo_feed_low_THI[sort(feed_cows)]) + 
  length(last_elo_feed_high_THI[sort(feed_cows)])
z_final_water_high_low_cor <- FisherZ(final_water_high_low_cor)
z_final_feed_high_low_cor <- FisherZ(final_feed_high_low_cor)
zobs_test45 <- 
  (z_final_water_high_low_cor-z_final_feed_high_low_cor)/
  sqrt(1/(n1-3)+1/(n2-3))
...
```{r optimization, message=FALSE, eval=FALSE}
...
ores_water_low_THI <-
  optimizek(elo_res_water_low_THI, krange = c(1, 100), resolution = 100)
ores_water_low_THI$best
ores_water_high_THI <-
  optimizek(elo_res_water_high_THI, krange = c(1, 100), resolution = 100)
ores_water_high_THI$best
ores_feed_low_THI <-
  optimizek(elo_res_feed_low_THI, krange = c(1, 100), resolution = 100)
ores_feed_low_THI$best
ores_feed_high_THI <-
  optimizek(elo_res_feed_high_THI, krange = c(1, 100), resolution = 100)
ores_feed_high_THI$best
```

### Figure 4.1 code:
```{r appendix Results-correlation plot 1, message=TRUE, eval=FALSE}
  df_f_w <- data.frame(x = rank(unname(sorted_last_elo_feed), ties.method = 
                                  'average'), 
                       y = rank(unname(sorted_last_elo_water), 
                                ties.method = 'average'))
  sp_df_f_w <- ggscatter(df_f_w, x = 'x', y = 'y',
   add = "reg.line",  # Add regression line
   add.params = list(color = "blue", fill = "lightgray"),
   ...
   ylab = "Dominance ranks of cows in Water Bin group"
            ...
            dominance ranks at Feed Bin and Water Bin (Overall)")
sp_df_f_w
```
### Figure 4.2 code:
```{r appendix Results-correlation plot 2, message=TRUE, eval=FALSE}
df_fh_wh <- data.frame(x = rank(unname(sorted_last_elo_feed_high_THI), 
                                ties.method = 'average'), 
                       y = rank(unname(sorted_last_elo_water_high_THI), 
                                ties.method = 'average'))
  sp_df_fh_wh <- ggscatter(df_fh_wh, x = 'x', y = 'y',
   ...
   ylab = "Dominance ranks of cows in Water Bin High THI group"
    ...
    labs(caption = "Figure 4.2: Correlation between the dominance ranks at 
         Feed Bin and Water Bin under High THI")
  
```
### Figure 4.3 code:
```{r appendix Results-correlation plots 3, message=TRUE, eval=FALSE}
df_fl_wl <- data.frame(x = rank(unname(sorted_last_elo_feed_low_THI), 
                                ties.method = 'average'), 
                       y = rank(unname(sorted_last_elo_water_low_THI), 
                                ties.method = 'average'))
  ...
  sp_df_fl_wl <- ggscatter(df_fl_wl, x = 'x', y = 'y',
   ...
   xlab = "Dominance ranks of cows in Feed Bin Low THI group",
   ylab = "Dominance ranks of cows in Water Bin Low THI group"
   ) + labs(caption = "Figure 4.3: Correlation between the dominance 
            ranks at Feed Bin and Water Bin under Low THI")
```

### Figure 4.4 code:
```{r appendix Results-correlation plots 4, message=TRUE, eval=FALSE}
df_wh_wl <- data.frame(x = rank(unname(
  last_elo_water_high_THI[sort(water_cows)]), 
                                ties.method = 'average'), 
                       y = rank(unname(
                         last_elo_water_low_THI[sort(water_cows)]), 
                                ties.method = 'average'))
  ...
  sp_df_wh_wl <- ggscatter(df_wh_wl, x = 'x', y = 'y',
     ...
     xlab = "Dominance ranks of cows in Water Bin High THI group",
     ylab = "Dominance ranks of cows in Water Bin Low THI group"
     ) + labs(caption = "Figure 4.4: Correlation between the dominance ranks 
              at Water Bin under High THI vs under Low THI")
```

### Figure 4.5 code:
```{r appendix Results-correlation plots 5, message=TRUE, eval=FALSE}
df_fh_fl <- data.frame(x = rank(unname(
  last_elo_feed_high_THI[sort(feed_cows)]), 
                                ties.method = 'average'), 
                       y = rank(unname(
                         last_elo_feed_low_THI[sort(feed_cows)]), 
                                ties.method = 'average'))
  sp_df_fh_fl <- ggscatter(df_fh_fl, x = 'x', y = 'y',
     add = "reg.line",  # Add regression line
     add.params = list(color = "blue", fill = "lightgray"),
     ...
     xlab = "Dominance ranks of cows in Feed Bin High THI group",
     ylab = "Dominance ranks of cows in Feed Bin Low THI group"
     ) + labs(caption = "Figure 4.5: Correlation between the dominance ranks 
              at Feed Bin under High THI vs under Low THI")
```
